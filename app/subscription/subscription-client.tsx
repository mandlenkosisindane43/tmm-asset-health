...
const plans = {
  Contractor: [
    { name: "Contractor Starter", fleet: "Up to 10 machines", sites: "1 site", note: "For small contractors starting with one operating site." },
    { name: "Contractor Multi-Site", fleet: "Up to 20 machines", sites: "Up to 3 sites", note: "Best fit for contractors managing several crews or sites.", recommended: true },
  ],
  Mine: [
    { name: "Mine Standard", fleet: "Up to 25 machines", sites: "1 mine site", note: "Designed for a mine site with a medium TMM fleet.", recommended: true },
    { name: "Mine Plus", fleet: "Up to 40 machines", sites: "1 mine site", note: "For larger single-site fleets with heavier reporting demand." },
    { name: "Mine Enterprise", fleet: "Up to 60 machines", sites: "Up to 2 sites", note: "For large operations requiring wider fleet and site coverage." },
    { name: "Enterprise+", fleet: "60+ machines", sites: "Multiple sites", note: "For complex multi-site mine operations." },
  ],
} as const;
...
export default function SubscriptionClient({ userEmail }: { userEmail: string }) {
  ...
  async function saveSelection() {
    ...
    const r = await fetch("/api/subscriptions", {
      ...
      body: JSON.stringify({ companyName, contactEmail, customerType: type, planName: selected.name, billingCycle: "Monthly" }),
    }).catch(() => null);
    ...
    if (!r?.ok) {
      ...
      setMessage("✓ Package request saved. Sindane Asset Solutions can now prepare the quotation and activate the licence after approval.");
      await load();
    }
    setSaving(false);
  }
      ...
      <div style={{maxWidth:1220,margin:"0 auto"}}>
        <div style={{display:"flex",justifyContent:"space-between",gap:20,alignItems:"center",marginBottom:28,flexWrap:"wrap"}}>
          <div>
            ...
            <h1 style={{fontSize:34,margin:"7px 0 8px"}}>Subscription Packages</h1>
            <p style={{margin:0,color:"#667085",maxWidth:760}}>Choose the contractor or mine package that matches your fleet. Commercial pricing is supplied through an official Sindane Asset Solutions quotation.</p>
          </div>
        ...
        <section style={{background:"white",border:"1px solid #dfe6ef",borderRadius:16,padding:20,marginBottom:20,boxShadow:"0 8px 24px rgba(22,34,51,.05)"}}>
          ...
          <div style={{display:"flex",gap:18,flexWrap:"wrap",justifyContent:"space-between",alignItems:"center",marginTop:18}}>
            <div>
              <b style={{display:"block",marginBottom:8}}>Customer type</b>
            ...
            <div style={{padding:"12px 16px",background:"#f7f9fc",border:"1px solid #dfe6ef",borderRadius:10}}>
              <small style={{display:"block",color:"#667085",fontWeight:800}}>BILLING</small>
              <b>Monthly subscription · quotation required</b>
            </div>
          </div>
        </section>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(250px,1fr))",gap:16}}>
          {visiblePlans.map(plan=><article key={plan.name} style={{background:"white",border:plan.recommended?"2px solid #172033":"1px solid #dfe6ef",borderRadius:16,padding:22,position:"relative",boxShadow:"0 8px 24px rgba(22,34,51,.05)"}}>
            ...
            <div style={{fontSize:23,fontWeight:900,margin:"18px 0 5px"}}>Official quotation required</div>
            <small style={{color:"#667085"}}>Pricing is prepared according to the selected fleet and site scope.</small>
            <div style={{margin:"20px 0",padding:"14px",background:"#f7f9fc",borderRadius:10,lineHeight:1.8}}>
              <b>{plan.fleet}</b><br/>
              <span>{plan.sites}</span><br/>
              <span>Implementation and onboarding included in the commercial quotation.</span>
            </div>
        ...
        {selected&&<section style={{marginTop:22,background:"#172033",color:"white",borderRadius:16,padding:22,display:"flex",justifyContent:"space-between",gap:20,alignItems:"center",flexWrap:"wrap"}}>
          <div>
            <small style={{fontWeight:800,opacity:.75}}>SELECTED PACKAGE</small>
          ...
          <div style={{textAlign:"right"}}>
            <b style={{fontSize:22}}>Quotation required</b>
            <div style={{fontSize:13,opacity:.8}}>SAS will confirm the commercial terms before licence activation.</div>
            <button onClick={saveSelection} disabled={saving} style={{marginTop:12,border:0,borderRadius:9,padding:"11px 17px",fontWeight:900,cursor:"pointer"}}>{saving?"Saving…":"Request this package"}</button>
          </div>
        </section>}
        ...
        <section style={{marginTop:28,background:"white",border:"1px solid #dfe6ef",borderRadius:16,padding:20}}>
          ...
          <div style={{overflowX:"auto",marginTop:12}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              <thead><tr>{["Company","Type / plan","Billing","Limits","Commercial status","Licence","Action"].map(h=><th key={h} style={{textAlign:"left",padding:"10px",borderBottom:"1px solid #e5e7eb",color:"#667085"}}>{h}</th>)}</tr></thead>
              ...
              </tr>):<tr><td colSpan={7} style={{padding:22,textAlign:"center",color:"#667085"}}>No company subscriptions saved yet.</td></tr>}</tbody>
            </table>
          </div>
        </section>

        <p style={{fontSize:12,color:"#7b879a",marginTop:20}}>Subscription prices are not displayed in the software. Sindane Asset Solutions issues an official quotation based on the selected package, fleet scope, sites and implementation requirements before licence activation.</p>
      </div>
    </main>
  );
}
