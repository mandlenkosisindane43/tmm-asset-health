...
import { handleTrialDemo } from "./trial-demo";
...
async function ensureSchema(env: Env) {
  await env.DB.prepare(
    `CREATE TABLE IF NOT EXISTS operational_alert_audit_v1 (
...
async function companyName(env: Env, cid: number) {
  const r = await first(env, "SELECT name FROM companies WHERE id=? LIMIT 1", [
    cid,
  ]);
  return txt(r?.name || `Company ${cid}`, 150);
}
async function recipients(env: Env, cid: number, kind: AlertKind) {
  const rows = await all(
    env,
    "SELECT email,breakdown,service_due AS serviceDue,missing_report AS missingReport,po,critical,active FROM alert_contacts_v3 WHERE company_id=? AND active=1 AND email IS NOT NULL AND trim(email)<>''",
    [cid],
  );
  const picked = rows
    .filter((r) => {
  ...
  const fallback = await all(
    env,
    "SELECT email FROM contractor_accounts WHERE company_id=? AND status='active' AND role IN ('company_admin','admin','manager','engineer') ORDER BY CASE role WHEN 'company_admin' THEN 1 WHEN 'admin' THEN 1 WHEN 'manager' THEN 2 ELSE 3 END LIMIT 10",
    [cid],
  );
  return [
    ...new Set(
      fallback.map((r) => lower(r.email)).filter((x) => x.includes("@")),
    ),
  ];
}
function emailHtml(
  ...
  action = "Open TMM Asset Health",
) {
  const rows = lines
    .map(
      ([k, v]) =>
        `<tr><td style="padding:7px 10px;color:#66747d;border-bottom:1px solid #edf1ef">${esc(k)}</td><td style="padding:7px 10px;font-weight:700;border-bottom:1px solid #edf1ef">${esc(v)}</td></tr>`,
    )
    .join("");
...
async function deliver(
  ...
  if (!env.RESEND_API_KEY) {
    ...
    try {
      ...
      if (res.ok) {
        status = "sent";
        if (data.id) providerIds = [String(data.id)];
      } else
        error = String(data.message || `Email provider returned ${res.status}`);
    } catch (e) {
...
async function scanCompany(env: Env, cid: number) {
  ...
  const events = await all(
    env,
    "SELECT id,fleet_number AS fleet,severity,system_name AS system,component,description,opened_at AS openedAt,downtime_hours AS downtime,status FROM events WHERE company_id=? AND lower(status)<>'closed' ORDER BY id DESC LIMIT 300",
    [cid],
  );
  for (const e of events) {
    ...
    await deliver(
      ...
      emailHtml(
        company,
        `${sev === "critical" ? "Critical" : "High-priority"} machine condition`,
        sev === "critical" ? "CRITICAL" : "HIGH",
        [
          ["Machine", e.fleet],
          ["System", e.system],
          ["Component", e.component],
          ["Fault / condition", e.description],
          ["Opened", e.openedAt],
          ["Downtime (h)", num(e.downtime).toFixed(2)],
          ["Status", e.status],
        ],
      ),
    );
  ...
  for (const m of machines) {
    ...
    await deliver(
      ...
      `${state} SERVICE · ${txt(m.fleet, 80)} · ${company}`,
      emailHtml(
        company,
        `Maintenance service ${state.toLowerCase()}`,
        remaining <= 0 ? "CRITICAL" : "SERVICE ALERT",
        [
          ["Machine", m.fleet],
          ["Type", m.category],
          ["Site", m.site],
          ["Hour meter", num(m.hours).toFixed(1)],
          ["Service due at", num(m.nextService).toFixed(1)],
          ["Hours remaining", remaining.toFixed(1)],
  ...
  const pos = await all(
    env,
    "SELECT id,order_number AS orderNumber,supplier,description,expected_delivery AS expectedDelivery,actual_delivery AS actualDelivery,order_status AS orderStatus,responsible_person AS responsiblePerson,reminder_email AS reminderEmail,fleet_number AS fleet FROM purchase_orders WHERE company_id=? AND reminder_email=1 AND expected_delivery IS NOT NULL AND trim(expected_delivery)<>''",
    [cid],
  );
  for (const p of pos) {
    if (txt(p.actualDelivery)) continue;
    const state = lower(p.orderStatus);
    if (["delivered", "received", "cancelled", "closed"].includes(state))
      continue;
    ...
    await deliver(
      ...
      "po",
      `po:${cid}:${p.id}:${bucket}`,
      `PO DELIVERY ${label} · ${txt(p.orderNumber, 80)} · ${company}`,
      emailHtml(
        company,
        `Purchase order delivery ${label.toLowerCase()}`,
        d < 0 ? "CRITICAL" : "PO / PARTS ALERT",
        [
          ["PO", p.orderNumber],
          ["Supplier", p.supplier],
          ...
          ["Responsible person", p.responsiblePerson || "—"],
  ...
  const repeats = await all(
    env,
    "SELECT fleet_number AS fleet,system_name AS system,component,COUNT(*) AS failures,MAX(id) AS latestId FROM events WHERE company_id=? AND datetime(created_at)>=datetime('now','-30 days') GROUP BY fleet_number,system_name,component HAVING COUNT(*)>=3 ORDER BY failures DESC LIMIT 50",
    [cid],
  );
  for (const r of repeats) {
    await deliver(
      ...
      `REPEAT FAILURE · ${txt(r.fleet, 80)} · ${company}`,
      emailHtml(company, "Repeated equipment failure detected", "CRITICAL", [
        ["Machine", r.fleet],
        ["System", r.system],
        ["Component", r.component],
        ["Failures in last 30 days", r.failures],
        ["Action", "Engineering review / root-cause investigation recommended"],
      ]),
    );
  }
}
...
export default {
  async fetch(
    ...
    const trial = await handleTrialDemo(req, env);
    ...
    if (cid && res.status < 400)
      ctx.waitUntil(
        scanCompany(env, cid).catch((e) =>
          console.error("operational alert scan failed", e),
        ),
      );
    return res;
  },
  async scheduled(
    ...
    ctx.waitUntil(
      scanAll(env).catch((e) =>
        console.error("scheduled operational alert scan failed", e),
      ),
    );
  },
};
