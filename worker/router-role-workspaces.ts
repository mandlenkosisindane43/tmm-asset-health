...
async function session(request: Request, env: Env): Promise<Session | null> {
  ...
  try {
    ...
    if (new Date(String(row.sessionExpires)).getTime() < Date.now())
      return null;
    return {
...
function hrefFor(view: string) {
  if (view === "trial-demo") return "/trial-demo";
...
function shell(s: Session, active: string, title: string, body: string) {
  const nav = [...navFor(s.role), ["trial-demo", "◈", "Trial Analysis"]];
...
function redirect(view: string, msg = "", tone = "ok") {
  ...
  return new Response(null, {
    status: 303,
    headers: { location: `/role?${q.toString()}`, "cache-control": "no-store" },
  });
}
...
async function common(env: Env, s: Session) {
  ...
  const events = await all(
    env,
    "SELECT id,fleet_number AS fleet,severity,system_name AS system,component,description,opened_at AS openedAt,closed_at AS closedAt,downtime_hours AS downtime,status,spares_status AS spares FROM events WHERE company_id=? ORDER BY id DESC LIMIT 500",
    [cid],
  );
  const work = await all(
    env,
    "SELECT id,fleet_number AS fleet,title,priority,assigned_to AS assignedTo,due_at AS dueAt,status,created_at AS createdAt FROM work_orders WHERE company_id=? ORDER BY id DESC LIMIT 400",
    [cid],
  );
  const production = await all(
    env,
    "SELECT report_date AS date,fleet_number AS fleet,shift_hours AS shiftHours,planned_downtime AS planned,unplanned_downtime AS unplanned,operating_hours AS operating,productive_hours AS productive,tonnes FROM production_records WHERE company_id=? ORDER BY report_date DESC LIMIT 600",
    [cid],
  );
...
function machineTable(rows: Row[], editable = false) {
  if (!rows.length)
    return `<div class="empty">No machines registered for this company.</div>`;
...
function workTable(rows: Row[], s: Session, editable = false) {
  ...
  return `<table class="table"><thead><tr><th>WO</th><th>Machine</th><th>Job</th><th>Priority</th><th>Assigned</th><th>Due</th><th>Status</th>${editable ? "<th>Update</th>" : ""}</tr></thead><tbody>${rows
    .map(
      (w) =>
        `<tr><td>WO-${num(w.id)}</td><td><b>${esc(w.fleet)}</b></td><td>${esc(w.title)}</td><td><span class="pill ${lower(w.priority) === "high" || lower(w.priority) === "critical" ? "red" : lower(w.priority) === "medium" ? "amber" : ""}">${esc(w.priority)}</span></td><td>${esc(w.assignedTo || "Unassigned")}</td><td>${esc(
          String(w.dueAt || "")
            .slice(0, 16)
            .replace("T", " "),
...
function eventTable(rows: Row[], editable = false) {
  if (!rows.length)
    return `<div class="empty">No faults or condition events found.</div>`;
  return `<table class="table"><thead><tr><th>Machine</th><th>Severity</th><th>System</th><th>Description</th><th>Opened</th><th>Downtime h</th><th>Status</th>${editable ? "<th>Action</th>" : ""}</tr></thead><tbody>${rows
    .map(
      (e) =>
        `<tr><td><b>${esc(e.fleet)}</b></td><td><span class="pill ${["critical", "high"].includes(lower(e.severity)) ? "red" : lower(e.severity) === "medium" ? "amber" : ""}">${esc(e.severity)}</span></td><td>${esc(e.system)}</td><td>${esc(e.description)}</td><td>${esc(
          String(e.openedAt || "")
            .slice(0, 16)
            .replace("T", " "),
...
async function renderRolePage(request: Request, env: Env, s: Session) {
  ...
  if (!allowed.has(view))
    return html(
      shell(
        ...
        "dashboard",
        "Access restricted",
        `${heading(s, "Access restricted", "This page is not assigned to your role.")}<div class="panel">Return to your dashboard.</div>`,
      ),
      403,
    );
  if (view === "dashboard")
    return new Response(null, {
      status: 302,
      headers: { location: "/contractor" },
    });
  if (view === "licence")
    return new Response(null, {
      status: 302,
      headers: { location: "/contractor-licence" },
    });
  const c = await common(env, s);
  const note = flash(url);

  if (
    ["fleet", "machines", "assets", "sites", "health", "compliance"].includes(
      view,
    )
  ) {
    const available = c.machines.filter(
      (m) => !["down", "maintenance", "inactive"].includes(lower(m.status)),
    ).length;
    const down = c.machines.filter((m) => lower(m.status) === "down").length;
    const maint = c.machines.filter(
      (m) => lower(m.status) === "maintenance",
    ).length;
    const sites = [
    ...
    if (view === "sites")
      main += `<div class="grid g3">${sites
        .map((site) => {
          const ms = c.machines.filter(
            (m) => String(m.site || "Main Site") === site,
          );
          const av = ms.filter(
            (m) => !["down", "maintenance"].includes(lower(m.status)),
          ).length;
          return `<div class="panel"><h2>${esc(site)}</h2><div class="mini"><span>Machines</span><b>${ms.length}</b></div><div class="mini"><span>Ready</span><b class="green">${av}</b></div><div class="mini"><span>Readiness</span><b>${ms.length ? ((av / ms.length) * 100).toFixed(1) : "0.0"}%</b></div></div>`;
        })
        .join("")}</div>`;
    else
      main += `<div class="panel">${machineTable(c.machines, s.role === "engineer" && (view === "machines" || view === "fleet"))}</div>`;
    if (view === "health" || view === "compliance")
      main += `<div class="panel section-gap"><h2>Current faults affecting asset health</h2>${eventTable(c.events.slice(0, 50), false)}</div>`;
    return html(
      shell(
        s,
        view,
        view === "fleet" ? "Fleet Overview" : roleTitle(s.role) + " Workspace",
        main,
      ),
    );
  }
  ...
  if (["workorders", "jobs", "work", "maintenance"].includes(view)) {
    let rows = c.work;
    if (s.role === "mechanic") {
      const me = s.fullName.toLowerCase(),
        mail = s.email.toLowerCase();
      rows = rows.filter((w) => {
        const a = String(w.assignedTo || "").toLowerCase();
        return !a || a === me || a === mail;
      });
    }
    const editable = ["engineer", "supervisor", "mechanic"].includes(s.role);
    const open = rows.filter(
      (w) => !["closed", "completed", "cancelled"].includes(lower(w.status)),
    ).length;
  ...
  if (["faults", "condition", "alerts"].includes(view)) {
    ...
    const critical = open.filter((e) =>
      ["critical", "high"].includes(lower(e.severity)),
    ).length;
    ...
    return html(
      shell(
        s,
        view,
        view === "condition" ? "Condition Monitoring" : "Faults & Alerts",
        main,
      ),
    );
  }
  ...
  if (view === "inspections") {
    const inspections = await all(
      env,
      "SELECT id,machine,inspection_type AS type,result,notes,inspector_name AS inspector,created_at AS createdAt FROM role_inspections_v4 WHERE company_id=? ORDER BY id DESC LIMIT 200",
      [s.companyId],
    );
  ...
  if (view === "checklists") {
      ...
      return html(
        shell(
          ...
          heading(
            s,
            "Access restricted",
            "Checklists are a Mechanic workspace.",
          ),
        ),
        403,
      );
    const rows = await all(
      env,
      "SELECT id,machine,item,status,notes,updated_at AS updatedAt FROM role_checklists_v4 WHERE company_id=? AND account_id=? ORDER BY id DESC LIMIT 200",
      [s.companyId, s.accountId],
    );
  ...
  if (view === "parts" || view === "inventory") {
    const rows = await all(
      env,
      "SELECT id,machine,part_name AS part,quantity,status,requester_name AS requester,created_at AS createdAt FROM role_parts_requests_v4 WHERE company_id=? ORDER BY id DESC LIMIT 250",
      [s.companyId],
    );
    const canAdd = ["engineer", "supervisor", "mechanic"].includes(s.role);
  ...
  if (view === "shift") {
    const notes = await all(
      env,
      "SELECT shift_name AS shift,note,created_at AS createdAt FROM role_shift_notes_v4 WHERE company_id=? ORDER BY id DESC LIMIT 100",
      [s.companyId],
    );
  ...
  if (view === "permits") {
    const permits = await all(
      env,
      "SELECT id,permit_type AS type,machine,reference,status,created_at AS createdAt FROM role_permits_v4 WHERE company_id=? ORDER BY id DESC LIMIT 200",
      [s.companyId],
    );
  ...
  if (view === "documents") {
    const docs = await all(
      env,
      "SELECT id,file_name AS fileName,category,content_type AS contentType,size_bytes AS sizeBytes,created_at AS createdAt FROM contractor_documents WHERE company_id=? ORDER BY id DESC LIMIT 250",
      [s.companyId],
    );
  ...
  if (view === "reports") {
    const hist = await all(
      env,
      "SELECT report_type AS type,period_key AS period,title,status,published_at AS publishedAt FROM report_history_v3 WHERE company_id=? ORDER BY published_at DESC LIMIT 100",
      [s.companyId],
    );
  ...
  return html(
    shell(
      ...
      "dashboard",
      "Workspace",
      heading(s, "Workspace", "This module is being prepared."),
    ),
  );
}
...
async function handleAction(request: Request, env: Env, s: Session) {
  ...
  if (action === "machine-update") {
    if (s.role !== "engineer")
      return redirect("machines", "Engineer permission is required.", "err");
    const id = num(f.get("id")),
      status = lower(f.get("status")),
      next = String(f.get("nextService") || "").trim();
    await env.DB.prepare(
      "UPDATE machines SET status=?,next_service_hours=? WHERE id=? AND company_id=?",
    )
      .bind(status, next === "" ? null : num(next), id, cid)
      .run();
  ...
  if (action === "work-create") {
    ...
    const fleet = txt(f.get("fleet"), 120),
      title = txt(f.get("title"), 300),
      priority = lower(f.get("priority")) || "medium",
      assigned = txt(f.get("assignedTo"), 160),
      due = txt(f.get("dueAt"), 40);
    if (!fleet || !title)
      return redirect(
        s.role === "supervisor" ? "jobs" : "workorders",
        "Machine and job are required.",
        "err",
      );
    await env.DB.prepare(
      "INSERT INTO work_orders(company_id,fleet_number,title,priority,assigned_to,due_at,status,created_at) VALUES(?,?,?,?,?,?,?,?)",
    )
      .bind(
        cid,
        fleet,
  ...
  if (action === "work-update") {
    ...
    const row = await first(
      env,
      "SELECT id,assigned_to AS assignedTo FROM work_orders WHERE id=? AND company_id=?",
      [id, cid],
    );
    if (!row)
      return redirect(
        s.role === "mechanic" ? "jobs" : "workorders",
        "Work order not found.",
        "err",
      );
    if (s.role === "mechanic") {
      const a = String(row.assignedTo || "").toLowerCase(),
        me = s.fullName.toLowerCase(),
        mail = s.email.toLowerCase();
      if (a && a !== me && a !== mail)
        return redirect(
          "jobs",
          "This work order is assigned to another person.",
          "err",
        );
      await env.DB.prepare(
        "UPDATE work_orders SET status=?,assigned_to=COALESCE(NULLIF(assigned_to,''),?) WHERE id=? AND company_id=?",
      )
        .bind(status, s.fullName, id, cid)
        .run();
    ...
    await env.DB.prepare(
      "UPDATE work_orders SET status=?,assigned_to=? WHERE id=? AND company_id=?",
    )
      .bind(status, assigned || null, id, cid)
      .run();
    return redirect(
      s.role === "supervisor" ? "jobs" : "workorders",
      "Work order updated.",
  ...
  if (action === "event-status") {
    if (!["engineer", "supervisor"].includes(s.role))
      return redirect(
        "faults",
        "You do not have permission to close faults.",
        "err",
      );
    const id = num(f.get("id")),
      status = lower(f.get("status"));
    ...
    await env.DB.prepare(
      "UPDATE events SET status=?,closed_at=? WHERE id=? AND company_id=?",
    )
      .bind(status, closed, id, cid)
      .run();
    return redirect(
      s.role === "supervisor" ? "alerts" : "faults",
      "Fault status updated.",
    );
  }
  if (action === "inspection-add") {
    if (!["engineer", "supervisor", "mechanic"].includes(s.role))
      return redirect("inspections", "Not permitted.", "err");
    const machine = txt(f.get("machine"), 120),
      type = txt(f.get("type"), 160),
      result = lower(f.get("result")) || "pass",
      notes = txt(f.get("notes"), 800);
    if (!machine || !type)
      return redirect(
        "inspections",
        "Machine and inspection type are required.",
        "err",
      );
    await env.DB.prepare(
      "INSERT INTO role_inspections_v4(company_id,machine,inspection_type,result,notes,inspected_by,inspector_name,created_at) VALUES(?,?,?,?,?,?,?,?)",
    )
      .bind(
        cid,
        machine,
  ...
  if (action === "checklist-add") {
    if (s.role !== "mechanic")
      return redirect("checklists", "Mechanic permission is required.", "err");
    const machine = txt(f.get("machine"), 120),
      item = txt(f.get("item"), 300),
      notes = txt(f.get("notes"), 600);
    if (!machine || !item)
      return redirect(
        "checklists",
        "Machine and checklist item are required.",
        "err",
      );
    await env.DB.prepare(
      "INSERT INTO role_checklists_v4(company_id,account_id,machine,item,status,notes,updated_at) VALUES(?,?,?,?,?,?,?)",
    )
      .bind(cid, s.accountId, machine, item, "pending", notes || null, isoNow())
      .run();
  ...
  if (action === "checklist-update") {
    if (s.role !== "mechanic")
      return redirect("checklists", "Mechanic permission is required.", "err");
    await env.DB.prepare(
      "UPDATE role_checklists_v4 SET status=?,updated_at=? WHERE id=? AND company_id=? AND account_id=?",
    )
      .bind(
        lower(f.get("status")),
        isoNow(),
  ...
  if (action === "part-add") {
    if (!["engineer", "supervisor", "mechanic"].includes(s.role))
      return redirect("parts", "Not permitted.", "err");
    const machine = txt(f.get("machine"), 120),
      part = txt(f.get("part"), 240),
      quantity = Math.max(0.01, num(f.get("quantity"), 1));
    if (!machine || !part)
      return redirect("parts", "Machine and part are required.", "err");
    await env.DB.prepare(
      "INSERT INTO role_parts_requests_v4(company_id,machine,part_name,quantity,status,requested_by,requester_name,created_at) VALUES(?,?,?,?,?,?,?,?)",
    )
      .bind(
        cid,
        machine,
  ...
  if (action === "shift-note") {
    if (s.role !== "supervisor")
      return redirect("shift", "Supervisor permission is required.", "err");
    const shift = txt(f.get("shift"), 80),
      note = txt(f.get("note"), 1200);
    if (!note) return redirect("shift", "Enter a shift note.", "err");
    await env.DB.prepare(
      "INSERT INTO role_shift_notes_v4(company_id,account_id,shift_name,note,created_at) VALUES(?,?,?,?,?)",
    )
      .bind(cid, s.accountId, shift || "Shift", note, isoNow())
      .run();
    return redirect("shift", "Shift note saved.");
  }
  if (action === "permit-add") {
    if (s.role !== "supervisor")
      return redirect("permits", "Supervisor permission is required.", "err");
    const type = txt(f.get("type"), 160),
      machine = txt(f.get("machine"), 120),
      ref = txt(f.get("reference"), 160);
    if (!type) return redirect("permits", "Permit type is required.", "err");
    await env.DB.prepare(
      "INSERT INTO role_permits_v4(company_id,permit_type,machine,reference,status,created_by,created_at) VALUES(?,?,?,?,?,?,?)",
    )
      .bind(
        cid,
        type,
...
function rewriteDashboardNav(body: string, role: string) {
  ...
  for (const [label, view] of mappings[role] || []) {
    const rx = new RegExp(
      `href="[^"]*"([^>]*)><span>([^<]*)<\\/span>${label.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}<\\/a>`,
      "g",
    );
    body = body.replace(
      rx,
      `href="/role?view=${view}"$1><span>$2</span>${label}</a>`,
    );
  }
  return body;
}
    ...
    if (path === "/role" || path === "/role/action") {
      ...
      if (!s)
        return Response.redirect(
          new URL("/contractor-login", request.url).toString(),
          302,
        );
      if (!licenceActive(s))
        return Response.redirect(
