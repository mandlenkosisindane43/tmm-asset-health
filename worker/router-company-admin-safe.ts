...
async function fleetUpgrade(body:string,env:Env,a:Row|null){
 ...
 const tableRows=rows.map(r=>{
   ...
   return `<tr><td><b>${esc(r.fleet)}</b></td><td>${hm.toLocaleString("en-ZA",{maximumFractionDigits:1})} h</td><td>${next.toLocaleString("en-ZA",{maximumFractionDigits:1})} h</td><td><b>${remaining.toLocaleString("en-ZA",{maximumFractionDigits:1})} h</b></td><td><span style="display:inline-flex;align-items:center;gap:6px;padding:5px 8px;border-radius:999px;background:${state.bg};color:${state.fg};font-weight:800">${state.dot} ${state.label}</span></td></tr>`;
 }).join("");
